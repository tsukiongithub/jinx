...
const NavLink = ({ href, text, router }: { href: string; text: string; router: NextRouter }) => {
	...
	return (
		<Link
			href={href}
			className={`${isActive ? "text-red-500" : "text-white"} font-lora animated-underline mr-4 py-4 px-4 text-lg font-medium transition-colors after:bg-red-800 last:mr-0 hover:text-red-700`}
		>
			{text}
		</Link>
	);
